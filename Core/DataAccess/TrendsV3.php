...
class Trends extends Database{
    ...
    function getChannelsTimeOfView($firstDate, $lastDate, $limit, $OrderBy){
        try {          
            $this->connect();
            $this->select("channelStatistics","SUM(horas) as HORAS, nombre_canal as Canal",
                          "",
                          "","","",
                          "date_format(`fecha_inicio`, '%Y-%m-%d') >= '$firstDate' AND date_format(`fecha_inicio`, '%Y-%m-%d') <= '$lastDate'","","nombre_canal","$OrderBy","$limit");
    ...
    function getLocationsTimeOfView($firstDate, $lastDate, $limit, $OrderBy){
        try {          
            $this->connect();
            $this->select("locationStatistics","SUM(horas) as HORAS, Cuenta as LOCATION",
                          "",
                          "","","",
                          "date_format(`fecha_inicio`, '%Y-%m-%d') >= '$firstDate' AND date_format(`fecha_inicio`, '%Y-%m-%d') <= '$lastDate'","","Cuenta","$OrderBy","$limit");
