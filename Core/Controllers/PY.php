 ...
 */
    ...
    $mediaTypeParameter  = !empty($_POST['mediaTypeParameter']) ? $_POST['mediaTypeParameter'] : 'videos';  
    $compressionStatus   = !empty($_POST['compressionStatus']) ? $_POST['compressionStatus'] : 'terminated';  
...
switch ($Option){
    ...
    break;
    case 'GetMediaType':
        $GuideDays = $ConfigData->getConfigByName('MediaType');
        array_push($ArrayEPGInfo, array('MediaType' => $GuideDays));
        echo json_encode($ArrayEPGInfo);
    break;
    case 'GetImageInterval':
        $GuideDays = $ConfigData->getConfigByName('imageInterval');
        $aux       = $GuideDays*1000;
        array_push($ArrayEPGInfo, array('Interval' => $aux));
        echo json_encode($ArrayEPGInfo);
    break;   
    case 'GetDigitalMediaType':
        $GuideDays = $ConfigData->getConfigByName('digitalMediaType');
        array_push($ArrayEPGInfo, array('digitalMediaType' => $GuideDays));
        echo json_encode($ArrayEPGInfo);
    break;
    case 'GetDigitalImageInterval':
        $GuideDays = $ConfigData->getConfigByName('digitalImageInterval');
        $aux       = $GuideDays*1000;
        array_push($ArrayEPGInfo, array('Interval' => $aux));
        echo json_encode($ArrayEPGInfo);
    break;     
    case 'GetCompressionStatus':
        $GuideDays = $ConfigData->getConfigByName('compressionStatus');
        array_push($ArrayEPGInfo, array('compressionStatus' => $GuideDays));
        echo json_encode($ArrayEPGInfo);
    break;    
    case 'GetIdentifier':
    ...
    case 'GetLastReboot':
            ...
            echo json_encode($ArrayEPGInfo);
    break;
    case 'UpdateIntervalParameter':
            $NewPackage = array(
                'valor_parametro' => $intervalParameter,   
                ); 
            $PackagesData->updateParameterImageInterval($NewPackage);
    break;
    case 'UpdateMediaTypeParameter':
            $NewPackage = array(
                'valor_parametro' => $mediaTypeParameter,   
                ); 
            $PackagesData->updateParameterMediaType($NewPackage);
    break;
    case 'UpdateDigitalIntervalParameter':
            $NewPackage = array(
                'valor_parametro' => $intervalParameter,   
                ); 
            $PackagesData->updateParameterDigitalImageInterval($NewPackage);
    break;
    case 'UpdateDigitalMediaTypeParameter':
            $NewPackage = array(
                'valor_parametro' => $mediaTypeParameter,   
                ); 
            $PackagesData->updateParameterDigitalMediaType($NewPackage);
    break;
    case 'UpdateCompressionStatus':
                    $NewPackage = array(
                        'valor_parametro' => $compressionStatus,   
                        ); 
                    $PackagesData->updateCompressionStatus($NewPackage);
    break;
}