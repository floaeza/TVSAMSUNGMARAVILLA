...
pak = requests.post('http://172.22.22.10BBINCO/TV/Core/Controllers/PY.php', data=payload)
...
def start(day, pos):
    ...
    Version = requests.post('http://172.22.22.10BBINCO/TV/Core/Controllers/PY.php', data=payload)
    Ver = json.loads(Version.content)
    Ver = Ver[0]

    payload = {'Option': 'GetIdentifier'}
    Identifier = requests.post('http://172.22.22.10BBINCO/TV/Core/Controllers/PY.php', data=payload)
    IDF = json.loads(Identifier.content)
    IDF = IDF[0]
    
    payload = {'Option': 'GetOffsetZone'}
    Zone = requests.post('http://172.22.22.10BBINCO/TV/Core/Controllers/PY.php', data=payload)
    OffSetZone = json.loads(Zone.content)
    OffSetZone = OffSetZone[0]

    payload = {'Option': 'GetGatoTime'}
    GTime = requests.post('http://172.22.22.10BBINCO/TV/Core/Controllers/PY.php', data=payload)
    ...
    for ids in range(int(pakk['Pack']), paquetes):
        ...
        x = requests.post('http://172.22.22.10BBINCO/TV/Core/Controllers/PY.php', data=payload)
...
for pos in range(len(listDays)):
    ...
    req = requests.post('http://172.22.22.10BBINCO/TV/Core/Controllers/Packages.php', data=info)
